...
if (!cX.library) {
    cX.library = {
        ...
        stopSessionAnnotation: function(removeAnnoations) {
            ...
            for (var annotationName in currentAnnotations) {
                if (currentAnnotations.hasOwnProperty(annotationName)) {
                    ...
                    var removeAnnotationValue = removeAnnoations[annotationName];
        ...
        sendPageViewEvent: function(args) {
            ...
            var wantAutoRefresh = false;
            var isAutoRefresh = false;
            try {
                if (location.href && document.referrer && (location.href != '') && (location.href == document.referrer)) {
                    isAutoRefresh = true;
                    var wantAutoRefreshDomains = ['lokus.se', 'teknomedia.se', 'affarsliv.com', 'corren.se',
                        'folkbladet.se', 'gotland.net', 'helagotland.se', 'kuriren.nu', 'mvt.se', 'nsd.se',
                        'nt.se', 'pitea-tidningen.se', 'unt.se', 'uppsala.com', 'uppsalavimmel.se', 'vt.se'];
                    for (var m = 0; m < wantAutoRefreshDomains.length; m++) {
                        if (location.hostname.indexOf(wantAutoRefreshDomains[m]) > -1) {
                            wantAutoRefresh = true;
                        }
                    }
                    if (!wantAutoRefresh) {
                        return;
            ...
            if (this.m_previousPageViewReport !== pageView) {
                ...
                if (document.images || wantAutoRefresh) {
        ...
        _defaultFlashCreativeRenderer: function(adSpace, ad) {
            var width = adSpace.adUnitWidth;
            var height = adSpace.adUnitHeight;
        ...
        _defaultComboCreativeRenderer: function(adSpace, ad) {
            ...
            for (var k = 0; k < ad.creative.flashFiles.length; k++) {
                ...
                var flashAd = { creative: { source: flashFile.source }, clickUrl: ad.clickUrl };
        ...
        _randomString: function() {
            return '' + Math.round(Math.random() * 2147483647).toString(36) + (new Date().getTime()).toString(36);
        },
        ...
        _encodeUrlEncodedNameValuePairs: function(object) {
            ...
            for (var attrName in object) {
                if (object.hasOwnProperty(attrName)) {
                    ...
                    } else if (Object.prototype.toString.call(attrValue) === '[object Array]') {
        ...
        _sendMessageToChild: function(childFrame, childSrc, message, domain) {
            if (this.hasPostMessage()) {
                if (childFrame.contentWindow && childFrame.contentWindow.postMessage) {
                    ...
                    if (typeof cX._cXSMTCLogOnce === 'undefined' && this.m_scriptStartTime % 100 === 1) {
                        try {
                            this._logRemote({ hpm: true, cfcw: !!childFrame.contentWindow, cfcd: !!childFrame.contentDocument, cfcdpm: (!!childFrame.contentDocument && !!childFrame.contentDocument.postMessage), cfrs: childFrame.readyState});
                        } catch (e) { }
                        cX._cXSMTCLogOnce = true;
                    }
                }
            } else {
                ...
                if (typeof cX._cXSMTCLogOnce === 'undefined' && this.m_scriptStartTime % 100 === 1) {
                    this._logRemote({ hpm: false, hdpm: !!document.postMessage, hdae: !!document.attachEvent });
                    cX._cXSMTCLogOnce = true;
                }
        ...
        _onPollFragmentMessage: function() {
            ...
            // hashChange event support is less than postMesssage, so we need to poll the hash
